...
const UpdateProject = ({ post, user }) => {
  const [caroselImage, setCaroselImage] = useState(post.caroselImage);
  ...
  const formik = useFormik({
    initialValues: {
      caroselImage: post.caroselImage,
      projectName: post.projectName,
      price: post.price,
      maxUnit: post.maxUnit,
      openDay: new Date(post.openDay),
      closeDay: new Date(post.closeDay),
  ...
  async function handleCreate(values) {
    ...
    const response = await fetch("/api/posts/update-post", {
      ...
      body: JSON.stringify({ slug: post.slug, ...values, status: "PENDING" }),
    });
    if (response.ok) {
  ...
  const del = () => {
    const updatedItems = caroselImage.filter(
      (_item, itemIndex) => itemIndex !== carouselIndex
    );
    setCaroselImage(updatedItems);
  };
  return (
    <>
      {isShowUploader ? (
          ...
          <div className="flex justify-center items-center w-3/12">
            <Link href="/profile/user">
              <a /* onPress={() => navigation.navigate('Profilediver')} */>
                <img src={user.photoURL} style={{ height: 70, width: 70 }} />
              </a>
            </Link>
          </div>
        </div>
      ...
      <div className="p-2 border-t-[0.5px] border-[#f0f0f0]  mt-0.5  bg-white sticky bottom-0 ">
        ...
        >
          <div className="text-white font-semibold">Cập nhật dự án</div>
        </button>
      </div>
    </>
  );
